...
async function createSummary(hoursAgo = 1) {
  try {
    ...
    console.log(
      `   - AQI TB: ${Math.round(response.data.data.statistics.aqi.avg)}`
    );
    console.log(
      `   - Nhiệt độ TB: ${response.data.data.statistics.temperature.avg.toFixed(
        1
      )}°C`
    );
    console.log(`\n📝 AI Summary:\n`);
    ...
    if (
      ...
      console.log(
        "   Thử tạo summary cho giờ khác bằng: node test-create-summary.js <số_giờ_trước>"
      );
    }
  }
}
